  ...
  useEffect(() => {
    const fetchReports = async () => {
      ...
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/reports/all`,
      ...
      <div style={{ padding: '2rem', flex: 1 }}>
        ...
        {reports.map(report => (
          <div key={report._id} style={{
            background: '#1a3a5c', padding: '1rem',
            marginBottom: '1rem', borderRadius: '8px'
          }}>
            <h3 style={{ color: 'white' }}>{report.title}</h3>
            <p style={{ color: '#94a3b8' }}>
              By: {report.intern?.name} | Week: {report.week}
            </p>
            <p style={{ color: 'white' }}>{report.content}</p>
            <p style={{ color: report.status === 'reviewed' ? 'lightgreen' : 'orange' }}>
